        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT * from `course_list` where delete_flag = 0 and tutor_id = '{$_settings->userdata('id')}' order by `name` asc ");
					...
					?>
						<tr>
							...
							<td class="text-center">
								<img src="<?= validate_image($row['logo_path']) ?>" alt="Course Logo" class="img-thumbnail border border-dark p-0 rounded-0 course-logo">
							</td>
...
<script>
	$(document).ready(function(){
		...
		$('.view_data').click(function(){
			uni_modal("<i class='fa fa-bars'></i> Course Details","courses/view_course.php?id="+$(this).attr('data-id'))
		})
