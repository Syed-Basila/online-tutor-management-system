...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT i.*, CONCAT(t.lastname,', ',t.firstname , COALESCE(concat(' ', t.middlename), '')) as `tutor`, t.email  from `inquiry_list` i  inner join tutor_list t on i.tutor_id = t.id where i.id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        ...
        if(isset($course_id)){
            $meta_qry = $conn->query("SELECT * from `course_list` where id = '{$course_id}' ");
            if($meta_qry->num_rows > 0){
                foreach($meta_qry->fetch_array() as $k => $v){
                    $course[$k] = $v;
                }
            }
        }
            ...
            <div class="card-body">
                <div class="container-fluid">
                    <fieldset class="border-bottom pb-3">
                        <legend>Inquired For</legend>
                        <div class="row mx-0">
                            <div class="col-lg-3 col-md-4 col-sm-12 col-xs-12 p-0">
                                <img src="<?= validate_image(isset($course['logo_path']) ? $course['logo_path'] : '') ?>" class="course_logo border" alt="">
                            </div>
                            <div class="col-lg-9 col-md-8 col-sm-12 col-xs-12">
                                <div class="row m-0">
                                    <div class="col-lg-3 col-md-4 col-sm-5 border align-middle">Course</div>
                                    <div class="col-lg-9 col-md-8 col-sm-7 border align-middle"><?= isset($course['name']) ? $course['name'] : '' ?></div>
                                </div>
                                <div class="row m-0">
                                    <div class="col-lg-3 col-md-4 col-sm-5 border align-middle">Experience</div>
                                    <div class="col-lg-9 col-md-8 col-sm-7 border align-middle"><?= isset($course['experience']) ? $course['experience'] : '' ?></div>
                                </div>
                                <div class="row m-0">
                                    <div class="col-lg-3 col-md-4 col-sm-5 border align-middle">Description</div>
                                    <div class="col-lg-9 col-md-8 col-sm-7 border align-middle"><?= str_replace(['\n\r', '\n', '\r'], '', isset($course['description']) ? $course['description'] : '') ?></div>
                                </div>
                            </div>
                        </div>
                    </fieldset>
                    <fieldset>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                                <label for="" class="control-label text-muted">Inquiry For</label>
                                <div class="pl-4 font-weight-bolder"><?= isset($tutor) ? $tutor : '' ?> (<?= isset($email) ? $email : '' ?>)</div>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
