...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `tutor_list` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	<form action="" id="tutor-form">
		...
		<div class="form-group">
			...
			<option value="3" <?php echo isset($status) && $status == 3 ? 'selected' : '' ?>>Block</option>
			</select>
		</div>
	</form>
</div>
<script>
	$(document).ready(function(){
		$('#tutor-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=update_tutor_status",
